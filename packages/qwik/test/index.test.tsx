...
describe("reactive", () => {
	...
	it("klassed", async () => {
		...
		await utils.expectElementFireClick(
			...
			(after) => after.tagName("A").className(clsx("color-blue", ["extra", "classes", "reactive"]))
		);
	});
	...
	it("reklassed", async () => {
		...
		await utils.expectElementFireClick(
			fireEvent,
			getElement,
			(before) => before.tagName("BUTTON").className(clsx("x-1", ["extra", "classes"])),
			(after) => after.tagName("A").className(clsx("x-2", ["extra", "classes", "reactive"]))
		);
	});
	...
	it("reklassed signal", async () => {
		...
		await utils.expectElementFireClick(
			...
			getElement,
			(before) => before.tagName("BUTTON").className(clsx("x-1", ["extra", "classes"])),
			(after) => after.tagName("BUTTON").className(clsx("x-2", ["extra", "classes", "reactive"]))
		);
	});
});
...
describe("composed", () => {
	...
	const test = async (Composed: any) => {
		const { element, cleanup } = await utils.renderRoot(<Composed as="main" {...PROPS} color="blue" size="lg" disabled />);
