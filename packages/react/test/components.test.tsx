...
import { A, Button, ClassAWithoutRef, ClassButtonWithoutRef, ClassDivWithoutRef, Div, RequiredA, RequiredButton, RequiredDiv } from "./~res/components";
	...
	describe("klassed", () => {
		...
		describe("class without ref", () => {
			...
			describe("polymorphic", () => {
				it("A", () => {
					const Component = poly.klassed("span", KLASS_OPTIONS);
					utils.expectElementRoot(<Component {...PROPS} as={ClassAWithoutRef} id="id" />).tagName("A");
				});

				it("Button", () => {
					const Component = poly.klassed("span", KLASS_OPTIONS);
					utils.expectElementRoot(<Component {...PROPS} as={ClassButtonWithoutRef} id="id" />).tagName("BUTTON");
				});
			});
		});
	});
	...
	describe("reklassed", () => {
		...
		describe("class without ref", () => {
			...
			describe("polymorphic", () => {
				it("Div", () => {
					const Component = poly.reklassed("span", REKLASS_OPTIONS);
					utils.expectElementRoot(<Component {...PROPS} as={ClassDivWithoutRef} id="id" />).tagName("DIV");
				});
			});
		});
	});
